#!/usr/bin/pyhon3
""" This Module is parent class that will inherit
"""
import uuid
...
class BaseModel:
    ...
    def __str__(self):
        """ Return class id, name and attribute dictionary """
        class_name = "[" + self.__class__.__name__ + "]"
        dct = {k: v for (k, v) in self.__dict__.items() if (not v) is False}
        return class_name + " (" + self.id + ") " + str(dct)

    def save(self):
        """ Update time of updated_at """
        self.updated_at = datetime.now()
        storage.save()

    def to_dict(self):
        """ Create a new dictionary, adding a key and returning
